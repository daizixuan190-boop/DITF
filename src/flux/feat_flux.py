...
from flux_multiblock import aggregate_mixed_block_outputs
...
class Featurizer4Eval(Featurizer):
    ...
    @torch.no_grad()
    def forward_multi_block(self,
                args,
                img_tensor,
                caption="a photo of a image",
                category="image",
                timestep=261,
                block_indices=(24, 28, 32),
                ensemble_size=1,
                guidance=3.5):
        """Extract multiple double-/single-stream features in one shared-noise forward."""
        blocks = tuple(sorted(set(int(value) for value in block_indices)))
        if not blocks or any(block < 0 for block in blocks):
            raise ValueError("block_indices must contain non-negative block indices")

        img_tensor = img_tensor.unsqueeze(0).repeat(ensemble_size, 1, 1, 1).cuda()
        prompt_embeds, text_ids, vec = prepare_txt(
                bs=ensemble_size,
                t5=self.t5,
                clip=self.clip,
        ...
        device = img_tensor.device
        t = timestep / 1000
        latents = self.ae.encode(img_tensor).to(torch.bfloat16)
        noise = torch.randn_like(latents).to(device)
        latents_noisy = t * noise + (1.0 - t) * latents
        _, _, latent_h, latent_w = latents_noisy.shape
        img, img_ids = prepare(img=latents_noisy)
        ...
        return aggregate_mixed_block_outputs(
            model_output,
            blocks,
            latent_h,
            latent_w,
